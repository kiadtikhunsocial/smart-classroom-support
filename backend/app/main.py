...
@app.patch("/api/pm/plans/{plan_id}")
def update_pm_plan(plan_id: int, payload: PMPlanCreate, db: Session = Depends(get_db), user: User = Depends(require_roles("super_admin", "admin"))):
    """แก้ไขแผน PM (super_admin / admin)"""
    plan = db.get(PMPlan, plan_id)
    if not plan:
        raise HTTPException(status_code=404, detail="PM plan not found")
    plan.name = payload.name
    plan.device_type = payload.device_type
    plan.interval_days = payload.interval_days
    plan.checklist = json.dumps(payload.checklist or [], ensure_ascii=False)
...
def delete_pm_plan(plan_id: int, db: Session = Depends(get_db), user: User = Depends(require_roles("super_admin", "admin"))):
    """ลบแผน PM (super_admin / admin)"""
    plan = db.get(PMPlan, plan_id)
    if not plan:
        raise HTTPException(status_code=404, detail="PM plan not found")
    # ปลดงานที่อ้างอิงแผนนี้ (กัน FK)
    db.execute(text("UPDATE pm_tasks SET plan_id = NULL WHERE plan_id = :pid"), {"pid": plan_id})
    db.delete(plan); db.commit()
    return {"message": "PM plan deleted", "plan_id": plan_id}
